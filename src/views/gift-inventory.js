...
function row(movement) {
  ...
  return `<tr>
    ...
    <td><strong>${escapeHTML(recipient)}</strong>${movement.customer_code ? `<br><span class="pill" style="font-size:0.72rem; padding:1px 6px; background:#eff6ff; color:#1d4ed8;"><i class="ri-hashtag"></i> ${escapeHTML(movement.customer_code)}</span>` : ''}<small>${escapeHTML(movement.recipient_phone || '')}</small></td>
...
function movementFields(type, fixedItem = '') {
  ...
  return `<input type="hidden" name="movementType" value="${type}"><label><span>Quà tặng</span><select name="itemId" required><option value="">Chọn quà tặng</option>${options((overview.items || []).filter(x => x.active), 'id', x => `${x.name} · còn ${x.stock} ${x.unit}`, fixedItem)}</select></label><label><span>Số lượng</span><input name="quantity" type="number" min="1" value="1" required></label>${recipient ? `<label><span>Mã khách hàng *</span><input name="customerCode" required placeholder="Mã KH hoặc SĐT để kiểm tra trùng" autocomplete="off"></label><label><span>Họ tên người nhận *</span><input name="recipientName" required placeholder="Họ tên khách hàng"></label><label><span>Số điện thoại</span><input name="recipientPhone" inputmode="tel" placeholder="Số điện thoại khách"></label>` : ''}${manager && recipient ? `<label><span>PG thực hiện</span><select name="pgCode"><option value="">Support trực tiếp</option>${options(pgAccounts, 'employee_code', x => x.full_name || x.employee_code)}</select></label>` : ''}${recipient ? `<div class="gift-evidence full"><label class="gift-upload"><span><i class="ri-user-smile-line"></i> Ảnh khách nhận/lấy quà *</span><input name="customerImage" type="file" accept="image/*" capture="environment" required><small>Chụp rõ khách và sản phẩm được nhận.</small></label><label class="gift-upload"><span><i class="ri-receipt-line"></i> Ảnh bill / hóa đơn *</span><input name="receiptImage" type="file" accept="image/*" capture="environment" required><small>Chụp đủ nội dung bill hoặc biên lai đối soát.</small></label></div>` : ''}<label class="full"><span>Ghi chú</span><textarea name="note" rows="3" placeholder="Chương trình, lý do hoặc thông tin đối soát"></textarea></label>`;
}
