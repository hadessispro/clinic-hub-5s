  ...
  return `<section class="panel pg-support-data">
    ...
    <div class="table-wrap pg-lead-table-wrap"><table class="pg-lead-table"><thead><tr><th>STT</th><th>Khách hàng</th><th>PG tiếp nhận</th><th>Phân loại</th><th>Dịch vụ</th><th>Telesale</th><th>Trạng thái</th><th>Xác nhận đến / HH</th><th>Ngày nhập</th></tr></thead><tbody>
      ${rows.length ? rows.map((lead, index) => {
        ...
        return `<tr><td>${start + index}</td><td><strong>${escapeHTML(lead.full_name || '')}</strong><small>${escapeHTML(lead.phone || 'Chưa có SĐT')}</small></td><td><strong>${escapeHTML(lead.created_by_name || lead.created_by_pg || 'PG')}</strong><small>${escapeHTML(lead.created_by_pg || '')}</small></td><td><span class="pg-data-tag ${lead.data_class === 'net' ? 'is-net' : 'is-raw'}">${lead.data_class === 'net' ? `Data net${lead.net_level === 'advanced' ? ' · Chuyên sâu' : ' · Cơ bản'}` : 'Data thô'}</span></td><td>${escapeHTML(lead.service_interest || 'Chưa xác định')}</td><td>${oNguoiPhuTrach(lead.assigned_telesale_name, lead.assigned_telesale_id)}</td><td>${leadStatusPill(lead.status)}</td><td>${confirmation}</td><td>${lead.created_at ? new Date(lead.created_at).toLocaleString('vi-VN') : '—'}</td></tr>`;
